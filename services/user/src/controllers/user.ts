...
export const deleteProfilePic = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    const user = req.user;
    ...
    if (!user.profile_pic) {
      throw new ErrorHandler(400, "No profile picture to delete");
    }

    if (user.profile_pic_public_id) {
      await axios.post(`${process.env.UPLOAD_SERVICE}/api/utils/delete`, {
        public_id: user.profile_pic_public_id,
      });
    }

    const [updatedUser] = await sql`
    UPDATE users SET profile_pic = NULL, profile_pic_public_id = NULL WHERE user_id = ${user.user_id} RETURNING user_id, name, profile_pic;
    `;
