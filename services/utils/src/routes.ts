...
router.post("/upload", async (req, res) => {
  try {
    ...
    const isPdf = typeof buffer === "string" && buffer.startsWith("data:application/pdf");
    const resourceType = isPdf ? "raw" : "image";

    const cloud = await cloudinary.v2.uploader.upload(buffer, { resource_type: resourceType });

    let url = cloud.secure_url;
    ...
    try {
      ...
      if (!isOverloadedError(error) || isLastAttempt) {
        if (isOverloadedError(error)) {
          throw new Error(
            "The AI service is currently experiencing high demand. Please try again in a minute."
          );
        }
        throw error;
      }
